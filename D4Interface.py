...
st.set_page_config(page_title="At-Risk Student UI", layout="wide")

# ===== Title (UI name shown on the page) =====
st.title("At-Risk Student Early Warning System (Interface Prototype)")
st.caption("Upload → Preview/Select Target → Train/Evaluate → Predict → Export")
...
# Basic data summary for D4 interface documentation
...
# Preprocessing (Interface: module I/O)
...
if run_train:
    ...
    out = df.copy()
    out["predicted_label"] = y_all

    # confidence (if model supports proba)
